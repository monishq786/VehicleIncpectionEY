sap.ui.define([
	...
	return Controller.extend("VIR.controller.InspectionTemp", {
		...
		handleRouteMatched: function (oEvent) {
			if (oEvent.getParameter("name") === "InspectionTemp") {
		...
		onPressSave: function () {
			...
			// Save button function added on 29-01-25
		...
		onConfirmVisualTest: function () {
			...
			var aInteriorlist = this.getView().getModel("SearchViewModel").getData().Interiorlist;
			var aLightarr = this.getView().getModel("SearchViewModel").getData().LightList;
			var aBrakearr = this.getView().getModel("SearchViewModel").getData().BrakeList;
			var aAttachmentarr = this.getView().getModel("SearchViewModel").getData().AttachmentList;
			var Clubarray = []
			Clubarray = Clubarray.concat(aLightarr, aBrakearr, aAttachmentarr);
			for (var i = 0; i < Clubarray.length; i++) {
				Clubarray[i].LocationDefectArray = [];
				Clubarray[i].Selected = false;
				Clubarray[i].Visible = false;
				Clubarray[i].SubComments = "";
			}
			for (var i = 0; i < aInteriorlist.length; i++) {
				aInteriorlist[i].State = false;
			}
		...
		onPressAdpolice: function () {
			...
			this.getView().getModel("SearchViewModel").setProperty("/NewModel", "Fortuner");
